...
class GeneticAlgorithmType(IntEnum):
    ...
    @staticmethod
    def get_strategy(strategy: int, dataset: Dataset, mutation_threshold: float):
        if strategy == 1:
            return DarwinStrategy(dataset, mutation_threshold)
        if strategy == 2:
            return LamarckStrategy(dataset, mutation_threshold)
        return RegularStrategy(dataset)
...
class BaseStrategy:
    ...
    def fitness(self, samples: List[Sample]) -> List[float]:
        ...
        fitness_scores = [correctness_ratio(s, self.__dataset) for s in samples]
        return fitness_scores


class OptimizationStrategy(BaseStrategy):
    def __init__(self, dataset: Dataset, mutation_threshold: float) -> None:
...
class DarwinStrategy(OptimizationStrategy):
    def __init__(self, dataset: Dataset, mutation_threshold: float) -> None:
        super().__init__(dataset, mutation_threshold)
